...
async fn main_response_mapper(res: Response) -> Response {
    ...
    //with client error we need to build a new response
    let error_response = client_status_error
        .as_ref()
        .map(|(status_code, client_error)| {
            let client_error_body = json!({
              "error" : {
                "type" : client_error.as_ref(), //strum!
                "req_uuid" : uuid.to_string(),
              }
            });
            // build new response from the client error
            println!("-->Client_error_body: {client_error_body}");
            (*status_code, json!(client_error_body))
        });

    println!("-->server_log_line - {uuid:?} - Error : {service_error:?}");
